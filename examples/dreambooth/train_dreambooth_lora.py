...
def main(
    ...
    text_encoder_use_attention_mask: bool,
    validation_images: Optional[List[str]],
    class_labels_conditioning: Optional[str],
    rank: int,
):
    if with_prior_preservation:
    ...
    for name, attn_processor in unet.attn_processors.items():
        ...
        module = lora_attn_processor_class(hidden_size=hidden_size, cross_attention_dim=cross_attention_dim, rank=rank)
    ...
    if train_text_encoder:
        ...
        text_lora_parameters = LoraLoaderMixin._modify_text_encoder(text_encoder, dtype=torch.float32, rank=rank)
    ...
    # Optimizer creation
    params_to_optimize = (
        itertools.chain(unet_lora_parameters, text_lora_parameters) if train_text_encoder else unet_lora_parameters
    )
    ...
    if accelerator.is_main_process:
        tracker_config = copy.deepcopy(configs)
        ...
        if accelerator.is_main_process:
            if validation_prompt is not None and epoch % validation_epochs == 0:
                ...
                pipeline = DiffusionPipeline.from_pretrained(
                    ...
                    text_encoder=None if pre_compute_text_embeddings else accelerator.unwrap_model(text_encoder),
                    revision=revision,
    ...
    if accelerator.is_main_process:
        ...
        pipeline.load_lora_weights(output_dir, weight_name="pytorch_lora_weights.bin")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="configs/train_dreambooth_lora.yaml")
